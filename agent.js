...
const OPENAI_API_KEY = process.env.OPENAI_API_KEY || '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';
...
Generate a response that represents JAS's friction framework. Be specific, confrontational where appropriate, and connect it to his work. Keep it under 200 words.`;
  ...
  console.log('[Agent] 🤔 Generating intelligent response with GPT-4o...');
  
  const response = await callGPT4(prompt);
  
  if (response) {
    console.log('[Agent] 💡 Response generated by GPT-4o');
    return response;
  } else {
...
async function runAgent() {
  ...
  console.log('[Agent] Powered by GPT-4o (OpenAI)');
...
console.log('Powered by GPT-4o (OpenAI)');
